...
void setup_printer() {
    ...
    PWM_PORT = 0;
    PWM_DDR |= (1 << NOZZLE_HEAT_PIN) | (1 << BED_HEAT_PIN) |
               (1 << FAN1_CONTROL_PIN) | (1 << FAN2_CONTROL_PIN);

    EndStopsAndAnableStepsDDR &=
        ~((1 << EndstopX) | (1 << EndstopY) | (1 << EndstopZ));
    ...
    log_success("The printer has loaded!\n"
                ...
                "Height:%d\n"
                "Max command len:%d",
                PrinterName, ChipName, SIZE_X_MM, SIZE_Y_MM, SIZE_Z_MM,
                MaxCommandLen);
...
ISR(TIMER0_COMP_vect) {
    ...
    if ((UATRTimeOut / ticksInSecond) * 1000 >= UART_Timeout_MS) {
...
unsigned int get_delay_timer(float speed, int speedAt) {
    ...
    // UART_printf("Speed:%f !!!!!  speedAt:%d !!!!! sesult:%d",speed,speedAt,
    // round((speedAt / speed)));
...
void send_cur_information() {
    ...
    case 2:
        ...
        break;
    case 3:
        UART_send_command(EndOfData, BedTemp, iPrinter.tempBed,
                          iPrinter.BedPID.needValue);
        break;
    case 4:
        UART_send_command(EndOfData, ExtruderTemp, iPrinter.tempNozzle,
...
inline void execute_MCode(const char *command) {
    ...
    case M83:
        Command_M83();
        break;
    case EnableStepscommand:
        enable_steps();
        break;
    case DisableStepscommand:
        disable_steps();
        break;
    case TurnOnFan:
...
inline void set_temp_bed(uint8_t temp) { iPrinter.BedPID.needValue = temp; }

inline void set_temp_nozzle(int temp) { iPrinter.NozzlePID.needValue = temp; }
